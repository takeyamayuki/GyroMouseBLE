...
void loop() {
  ...
  if(bleMouse.isConnected()) {     //接続状況確認
    if(touchRead(T0)<30){
       bleMouse.move(-(z[1]-z[0])*x_kand, (y[1]-y[0])*y_kand, 0);
    }
    if(digitalRead(16)==0){
       bleMouse.click() ;
       delay(100);
...
void calcRotation(){
  ...
  //kalmanfilter
  kalAngleX = kalmanX.getAngle(acc_angle_x, gyro_angle_x, 1);
  kalAngleY = kalmanY.getAngle(acc_angle_y, gyro_angle_y, 1);
  //kalAngleZ = kalmanZ.getAngle(acc_z, gyro_angle_z, 1);
